<template>
  <Article>
    <h3>Bibliografia</h3>
    <Section>
      <li><b>A. Dragan - Niezwykle szczególna teoria względności</b></li>
      <li><b>J. Taylor - Mechanika klasyczna tom 1-2</b></li>
      <li><b>W. Kurnik - Wykłady z mechaniki ogólnej</b></li>

      <li>A. Wróblewski - Historia fizyki</li>
      <li>A. Wróblewski - Wstęp do fizyki tom 1-2</li>
      <li>D. Halliday, R. Resnick, J. Walker - Podstawy fizyki tom 1, 2, 4</li>
      <li>J. Leyko - Mechanika ogólna tom 1-2</li>
      <li>K. Tchoń, R. Muszyński - Mechanika analityczna</li>
      <li>L. Sokołowski - Szczególna teoria względności, notatki z wykładu</li>
      <li>
        L. Susskind, A. Friedman - Szczególna teoria względności i klasyczna
        teoria pola
      </li>
      <li><Nobel>Lew D. Landau</Nobel>, J. M. Lifszyc - Mechanika</li>
      <li><Nobel>Lew D. Landau</Nobel>, J. M. Lifszyc - Teoria pola</li>
      <li>M. Massalska, J. Massalski - Fizyka dla inżynierów tom 1-2</li>
      <li>P. Wiśniakowski - Mechanika teoretyczna</li>
      <li>
        <Nobel>Richard Feynman</Nobel>, R. Leighton, M. Sands - Feynmana wykłady
        z fizyki tom 1.1
      </li>
      <li>W. Królikowski, W. Rubinowicz - Mechanika teoretyczna</li>
      <li>W. Siuta - Mechanika techniczna</li>
      <li>W. Tybor, K. Kowalski - Mechanika</li>
    </Section>
    <h3>Uzupełnienie</h3>
    <Section>
      <li>A. Stawinoga, R. Bąk - Mechanika dla niemechaników</li>
      <li>E, Wittbrodt, S. Sawiak - Mechanika ogólna, teoria i zadania</li>
      <li>J. Misiak - Mechanika ogólna tom 1-2</li>
      <li>K. Stefański - Wstęp do mechaniki klasycznej</li>
      <li>M. Fayngold - Special relativity and how it works</li>
      <li>M. Wierzbicki - Mechanika klasyczna w zadaniach</li>
      <li>
        R. Buczkowski, A. Banaszek - Mechanika ogólna w ujęciu wektorowym i
        tensorowym
      </li>
      <li>R. Gutowski - Mechanika analityczna</li>
      <li>S. Pryputniewicz - Mechanika ogólna</li>
      <li>W. Biały - Mechanika z przykładami</li>
      <li>Z. Osiński - Mechanika ogólna tom 1-3</li>
      <li>Z. Towarek - Mechanika ogólna</li>
    </Section>
  </Article>
</template>
