export default {
  'Wstęp do fizyki': {
    'Filozofia fizyki': {
      'Dziedziny i zakres fizyki': 'Wstep/1/1.vue',
      'Historia fizyki': 'Wstep/1/2.vue',
      // 'Fizyka wśród innych nauk': 'Wstep/1/3.vue',
      // 'Filozofia nauki': 'Wstep/1/4.vue',
      // 'Zagadnienia filozofii fizyki': 'Wstep/1/5.vue',
    },
    'Wprowadzenie do fizyki': {
      'Ruch i jego przyczyny': 'Wstep/2/1.vue',
      'Ciepło, a statystyka': 'Wstep/2/2.vue',
      'Elektryczność i magnetyzm': 'Wstep/2/3.vue',
      'Tajemniczy świat kwantów': 'Wstep/2/4.vue',
      'Materiały i struktury': 'Wstep/2/5.vue',
      'O energii jądrowej': 'Wstep/2/6.vue',
      'Budowa materii': 'Wstep/2/7.vue',
      'W kosmos i dalej': 'Wstep/2/9.vue',
    },
  },
  Mechanika: {
    Kinematyka: {
      'Czas i przestrzeń': 'Mechanika/1/1.vue',
      'Tor ruchu': 'Mechanika/1/2.vue',
      'Elementarny opis ruchu': 'Mechanika/1/3.vue',
      'Spadek swobodny': 'Mechanika/1/4.vue',
      'Rzut poziomy i ukośny': 'Mechanika/1/5.vue',
      'Ruch po okręgu': 'Mechanika/1/6.vue',
      'Prędkość i przyspieszenie': 'Mechanika/1/7.vue',
      'Ruch krzywoliniowy': 'Mechanika/1/8.vue',
      'Krzywizna i skręcenie': 'Mechanika/1/9.vue',
      'Czasoprzestrzeń Galileusza': 'Mechanika/1/10.vue',
      Orbity: 'Mechanika/1/11.vue',
      Zadania: 'Mechanika/1/Z.vue',
      // Ciekawostki: 'Mechanika/1/C.vue',
    },
    'Sily w przyrodzie': {
      'Wprowadzenie do mechaniki': 'Mechanika/2/1.vue',
      'Siły i ruch': 'Mechanika/2/2.vue',
      'Reakcja ciał na siłę': 'Mechanika/2/3.vue',
      'Układy odniesienia': 'Mechanika/2/4.vue',
      'Ciężar i nacisk': 'Mechanika/2/5.vue',
      Tarcie: 'Mechanika/2/6.vue',
      'Siła dośrodkowa': 'Mechanika/2/7.vue',
      'Zasady dynamiki w układzie współrzędnych': 'Mechanika/2/8.vue',
      'Siły molekularne i jądrowe': 'Mechanika/2/9.vue',
      'Podstawy teorii pola': 'Mechanika/2/10.vue',
      'Równania pola': 'Mechanika/2/11.vue',
    },
    'Ruch pocisków i cząstek naładowanych': {
      'Opór powietrza': 'Mechanika/3/1.vue',
      //   'Tor i zasięg rzutu w ośrodku z oporem': 'Mechanika/2/2.vue',
      //   'Kwadratowa siła oporu powietrza': 'Mechanika/2/3.vue',
      //   'Ruch cząstki w polu magnetycznym': 'Mechanika/2/4.vue',
    },
    'Pęd i jego przemiany': {
      'Zasada zachowania pędu': 'Mechanika/4/1.vue',
      Zderzenia: 'Mechanika/4/2.vue',
      Rakieta: 'Mechanika/4/3.vue',
    },
    Energia: {
      'Energia kinetyczna i praca': 'Mechanika/5/1.vue',
      'Siły zachowawcze i energia potencjalna': 'Mechanika/5/2.vue',
      'Siła i potencjał': 'Mechanika/5/3.vue',
      // 'Energia w układach jednowymiarowych': 'Mechanika/5/4.vue',
      // 'Krzywoliniowe układy jednowymiarowe': 'Mechanika/5/5.vue',
      // 'Siły centralne': 'Mechanika/5/6.vue',
      // 'Energia oddziaływania dwóch cząstek': 'Mechanika/57.vue',
      // 'Energia układu wielu cząstek': 'Mechanika/5/8.vue',
    },
    // Obroty: {},
    // 'Układy nieinercjalne': {},
    // Równowaga: {},
    // 'Geometria mas': {},
    // Grawitacja: {},
  },
  // 'Mechanika Analityczna': {
  //   "Mechanika Lagrange'a": {},
  //   'Symetrie i zasady zachowania': {},
  //   'Ciało sztywne': {},
  //   'Teoria zderzeń': {},
  //   'Równania kanoniczne': {},
  //   'Mechanika nieliniowa i chaos': {},
  // },
  // Relatywistyka: {
  //   'Teoria względności': {},
  //   Czasoprzestrzeń: {},
  //   'Mechanika relatywistyczna': {},
  // },
}
